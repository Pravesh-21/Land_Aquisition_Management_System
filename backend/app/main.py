...
FastAPI Microservices Stack with Database User Authentication & RBAC
"""
...
# CORS Setup - explicit origins required when allow_credentials=True
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        "http://localhost:3000",
...
class Notification(BaseModel):
    id: str
    role: str
    category: str           # CASE_UPDATE | PAYMENT | HEARING | DOCUMENT | ALERT | WORKFLOW | SYSTEM
    title: str
    message: str
    timestamp: str
    is_read: bool = False
...
# =========================================================
# NOTIFICATIONS IN-MEMORY DATABASE (Seeded per role)
# =========================================================
...
NOTIFICATIONS_DB: Dict[str, List[dict]] = {
    "AGENCY": [
        ...
        {"id": str(uuid.uuid4()), "role": "AGENCY", "category": "DOCUMENT", "title": "Section 19 Notification Issued", "message": "District Collector Nagpur has issued Section 19 Declaration for NH-48 Phase 3. All landowners notified via CPGRAMS.", "timestamp": _ts(45), "is_read": False, "priority": "NORMAL", "action_url": "/dashboard/agency/documents", "icon": "description"},
        {"id": str(uuid.uuid4()), "role": "AGENCY", "category": "WORKFLOW", "title": "Forest Clearance Pending", "message": "MoEFCC Stage-II Forest Clearance for 14.2 ha is awaiting DFO signature. Project timeline at risk.", "timestamp": _ts(120), "is_read": True, "priority": "HIGH", "icon": "account_tree"},
    ...
    "LAO": [
        ...
        {"id": str(uuid.uuid4()), "role": "LAO", "category": "PAYMENT", "title": "Compensation Award Ready", "message": "RFCTLARR compensation of ₹47.3 Lakhs calculated for Sh. Rajendra Patel (Parcel MH-NG-1056). Awaiting Collector approval.", "timestamp": _ts(18), "is_read": False, "priority": "HIGH", "action_url": "/dashboard/lao/compensation", "icon": "payments"},
        ...
        {"id": str(uuid.uuid4()), "role": "LAO", "category": "DOCUMENT", "title": "SIA Objection Filed", "message": "Citizen Sh. Mohan Lal has filed a formal objection to the SIA report citing livelihood impact. Ref: OBJ-2024-881.", "timestamp": _ts(95), "is_read": True, "priority": "NORMAL", "icon": "description"},
        {"id": str(uuid.uuid4()), "role": "LAO", "category": "ALERT", "title": "Aadhaar Verification Failed", "message": "Aadhaar seeding failed for 3 landowners in Parcel cluster RJ-SK-0800 to 0810. Manual verification required.", "timestamp": _ts(180), "is_read": True, "priority": "HIGH", "icon": "warning"},
    ...
    "FOREST": [
        {"id": str(uuid.uuid4()), "role": "FOREST", "category": "DOCUMENT", "title": "Forest Land Diversion Request", "message": "AGENCY has submitted Forest Land Diversion request for 14.2 ha under NH-48. Stage-I Clearance documents attached.", "timestamp": _ts(12), "is_read": False, "priority": "HIGH", "action_url": "/dashboard/forest/clearances", "icon": "description"},
        ...
        {"id": str(uuid.uuid4()), "role": "FOREST", "category": "WORKFLOW", "title": "Wildlife Sanctuary Buffer Encroachment", "message": "YOLOv8 satellite audit detected potential encroachment within 500m buffer of Melghat Tiger Reserve. Site inspection ordered.", "timestamp": _ts(90), "is_read": False, "priority": "HIGH", "icon": "account_tree"},
    ...
    "COLLECTOR": [
        {"id": str(uuid.uuid4()), "role": "COLLECTOR", "category": "CASE_UPDATE", "title": "Section 23 Award Pending Approval", "message": "LAO Pune has submitted Section 23 Award for 7 parcels totalling ₹3.2 Cr. Statutory 60-day window expires in 8 days.", "timestamp": _ts(3), "is_read": False, "priority": "CRITICAL", "action_url": "/dashboard/collector/awards", "icon": "update"},
        {"id": str(uuid.uuid4()), "role": "COLLECTOR", "category": "HEARING", "title": "Section 11 Enquiry Scheduled", "message": "Public enquiry under Section 11 for NH-48 Phase 3 project scheduled for next Monday. 47 affected landowners notified.", "timestamp": _ts(25), "is_read": False, "priority": "HIGH", "icon": "gavel"},
        {"id": str(uuid.uuid4()), "role": "COLLECTOR", "category": "ALERT", "title": "Court Order Received", "message": "High Court of Bombay has issued stay order on possession of Parcel MH-NG-2041 pending litigation. Immediate halt required.", "timestamp": _ts(50), "is_read": False, "priority": "CRITICAL", "icon": "warning"},
        {"id": str(uuid.uuid4()), "role": "COLLECTOR", "category": "DOCUMENT", "title": "Section 19 Draft Ready", "message": "Legal team has prepared Section 19 Declaration draft for NH-48 Phase 3. Awaiting your signature.", "timestamp": _ts(130), "is_read": True, "priority": "NORMAL", "icon": "description"},
    ...
    "TEHSILDAR": [
        {"id": str(uuid.uuid4()), "role": "TEHSILDAR", "category": "CASE_UPDATE", "title": "New Cases Assigned", "message": "5 new land acquisition cases from Collector's office have been assigned to your docket. Deadline: 15 days.", "timestamp": _ts(10), "is_read": False, "priority": "HIGH", "action_url": "/dashboard/tehsildar", "icon": "update"},
        {"id": str(uuid.uuid4()), "role": "TEHSILDAR", "category": "HEARING", "title": "Mutation Hearing — RJ-SK-0892", "message": "Mutation hearing for Parcel RJ-SK-0892 confirmed for 10:30 AM. Landowner Sh. Vikram Yadav and his advocate will attend.", "timestamp": _ts(20), "is_read": False, "priority": "HIGH", "action_url": "/dashboard/tehsildar/hearing-manager", "icon": "gavel"},
        {"id": str(uuid.uuid4()), "role": "TEHSILDAR", "category": "DOCUMENT", "title": "Survey Report Submitted", "message": "Revenue Surveyor has submitted measurement report for 3 parcels in Sikar Tehsil. Verification pending.", "timestamp": _ts(70), "is_read": False, "priority": "NORMAL", "icon": "description"},
        {"id": str(uuid.uuid4()), "role": "TEHSILDAR", "category": "ALERT", "title": "Boundary Dispute — Parcel RJ-SK-0901", "message": "GIS overlay shows 0.3 ha boundary discrepancy between DILRMP records and satellite imagery. Ground-truthing required.", "timestamp": _ts(150), "is_read": True, "priority": "HIGH", "icon": "warning"},
    ...
    "CITIZEN": [
        {"id": str(uuid.uuid4()), "role": "CITIZEN", "category": "PAYMENT", "title": "Compensation Credited — ₹47.3 Lakhs", "message": "Your RFCTLARR compensation of ₹47,38,500 has been credited to your Aadhaar-linked bank account (XXXX 4920). Reference: AWD-2024-1056.", "timestamp": _ts(2), "is_read": False, "priority": "CRITICAL", "action_url": "/dashboard/citizen/compensation", "icon": "payments"},
        {"id": str(uuid.uuid4()), "role": "CITIZEN", "category": "HEARING", "title": "Hearing Notice — Section 15", "message": "You are required to appear before LAO Pune Division on 05-Sep-2024 at 11:00 AM regarding your objection. Bring original title documents.", "timestamp": _ts(15), "is_read": False, "priority": "HIGH", "action_url": "/dashboard/citizen/hearings", "icon": "gavel"},
        ...
        {"id": str(uuid.uuid4()), "role": "CITIZEN", "category": "DOCUMENT", "title": "Title Verification Complete", "message": "Your land title (Survey No. 142/B, Nagpur) has been verified and linked to your Aadhaar. Records are now digitized.", "timestamp": _ts(96), "is_read": True, "priority": "NORMAL", "icon": "description"},
        {"id": str(uuid.uuid4()), "role": "CITIZEN", "category": "ALERT", "title": "Grievance Status Update", "message": "Your CPGRAMS grievance (Ref: GRV-2024-4412) has been escalated to the District Collector. Expected resolution in 15 days.", "timestamp": _ts(200), "is_read": True, "priority": "NORMAL", "icon": "warning"},
        {"id": str(uuid.uuid4()), "role": "CITIZEN", "category": "SYSTEM", "title": "Rehabilitation Package Announced", "message": "Government has announced an additional R&R package including house site allotment under Section 31. Apply by 30-Sep-2024.", "timestamp": _ts(400), "is_read": True, "priority": "NORMAL", "icon": "info"},
    ],
}
...
def health_check():
    return {"status": "healthy", "database_users_count": len(USER_DATABASE), "postgis": "connected"}
...
def login_user(req: LoginRequest):
    ...
    provided_password = req.password_or_otp
